...
var FollowPrefix = "follow{user_id:%d; follow_id:%d}" //redis中follow的前缀

type FollowMicroservice struct {
}

func (microservice *FollowMicroservice) DealData(currEntity *RawLogEntity, commands *[]*RedisPipelineCommand) (err error) {
	...
	if currEntity.Action == "update" || currEntity.Action == "delete" {
		...
		for idx, row := range currEntity.Rows {
			...
			key := fmt.Sprintf(FollowPrefix, row[currEntity.HeaderMap["user_id"]], row[currEntity.HeaderMap["follow_id"]])
